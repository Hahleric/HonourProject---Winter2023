...
def create_robots(manual):
    ...
    for i in range(ROBOT_NUMBER):
        ...
        robot = r.Robot_5d(name, x, y, z, t, q)
...
def line_update(robot_list):
    ...
    for robot in robot_list:
        ...
        min_position = random_r[min_index].get_x(), random_r[min_index].get_y(), random_r[min_index].get_z(), random_r[min_index].get_t(), random_r[min_index].get_p()
        to_move_positions.append(min_position)
    for i in range(len(robot_list)):
        robot_list[i].move_to(to_move_positions[i][0], to_move_positions[i][1], to_move_positions[i][2], to_move_positions[i][3], to_move_positions[i][4])
...
def is_finished(robot_points):
    for i in range(len(robot_points)):
        if robot_points[i].get_x() != robot_points[0].get_x() or \
                ...
                robot_points[i].get_p() != robot_points[0].get_p():
            return False
    return True
