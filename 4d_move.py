...
def line_update(robot_list):
    ...
    for robot in robot_list:
        ...
        min_position = random_r[min_index].get_x(), random_r[min_index].get_y(), random_r[min_index].get_z(), random_r[min_index].get_t()
        to_move_positions.append(min_position)
    for i in range(len(robot_list)):
        robot_list[i].move_to(to_move_positions[i][0], to_move_positions[i][1], to_move_positions[i][2], to_move_positions[i][3])
...
def is_finished(robot_points):
    for i in range(len(robot_points)):
        if robot_points[i].get_x() != robot_points[0].get_x() or \
                ...
                robot_points[i].get_t() != robot_points[0].get_t():
            return False
    return True
